...
public static class UserDataSerializer<T>
{
    ...
    public static T LoadUserData(List<string> propertiesToRemove)
    {
        if (File.Exists(Path))
        {
            ...
            //respJObject.Descendants().OfType<JProperty>().Where(attr => attr.Name.Contains("MissionType")).ToList().ForEach(attr => attr.Remove());

            //return File.Exists(Path) ? JsonConvert.DeserializeObject<T>(File.ReadAllText(Path), new JsonSerializerSettings()
            //{
            //    TypeNameHandling = TypeNameHandling.Auto
            //}) : default;
        }
